...
{
public:
    TreeViewDemoItem (XmlElement& xml_, Value::Listener* parentPointer)
    : xml (xml_)
    {
        parent = parentPointer;
        selectedName.addListener(parent);
    ...
    void paintItem (Graphics& g, int width, int height)
    {
        ...
        if (isSelected())
        {
            ...
            selectedName = xml.getStringAttribute("Name", "Music");
        }
        ...
        g.drawText (//xml.getTagName(),
                    xml.getStringAttribute("Name", "Music"),
                    4, 0, width - 4, height,
        ...
        if (isNowOpen)
        {
            ...
            if (getNumSubItems() == 0)
            {
                ...
                forEachXmlChildElement (xml, child)
                {
                    ...
                    addSubItem (new TreeViewDemoItem (*child, parent));
                }
            }
        }
        else
    ...
    bool canBeSelected()
    {
        DBG("Checking can be selected = " << xml.getStringAttribute("Name", "Music") << " " << containsSubItems);
        if (containsSubItems)
        {
            return false;
        }
...
//            treeXml = parser.getDocumentElement();
            treeXml = singletonPlaylistsTree.createXml();
            jassert (treeXml != nullptr);
        }
        
        rootItem = new TreeViewDemoItem (*treeXml, this);
