...
//	DBG("ITunesLibrary to use changed");
	filteredDataList_ = dataList = library->getLibraryTree();
...
//    filteredArray.clear();
	...
	{
		filteredDataList_ = dataList;
		filteredNumRows = filteredDataList_.getNumChildren();
...
//        }
	...
	{
		filteredDataList_ = ValueTree (dataList.getType());
		...
		for (int e = 0; e < dataList.getNumChildren(); e++)
		{
			for (int i = 0; i < dataList.getChild (e).getNumProperties(); i++)
			{
				...
				{
//                    filteredArray.add (dataList.getChild (e));
					filteredDataList_.addChild (dataList.getChild(e).createCopy(), -1, 0);
...
void MusicLibraryTable::libraryChanged (ITunesLibrary* library)
{
	...
	{
		filteredDataList_ = dataList = currentLibrary->getLibraryTree();
		filteredNumRows = filteredDataList_.getNumChildren();
								   ...
								   bool rowIsSelected)
{
...

    {
        ...
        const ValueTree& rowElement (filteredDataList_.getChild (rowNumber));
...
void MusicLibraryTable::sortOrderChanged (int newSortColumnId, bool isForwards)
{
	if (newSortColumnId != 0)
	{
            ...
            || newSortColumnId == MusicColumns::Modified)
		{
			...
			filteredDataList_.sort (sorter, 0, false);
...
int MusicLibraryTable::getColumnAutoSizeWidth (int columnId)
{
	...
	for (int i = getNumRows(); --i >= 0;)
	{
        {
            ScopedLock sl (currentLibrary->getParserLock());
            const ValueTree& rowElement (filteredDataList_.getChild (i));
        ...
        {
            {
                ...
                const ValueTree& tree (filteredDataList_.getChild (currentlySelectedRows[i]));
